	...
	sih_haptic_t *sih_haptic)
{
	...
	if (gpio_is_valid(sih_haptic->chip_attr.irq_gpio) &&
		!sih_haptic->chip_attr.irq_gpio_flag) {
		ret = devm_gpio_request_one(dev, sih_haptic->chip_attr.irq_gpio,
			GPIOF_DIR_IN, "sih_haptic_irq");
		if (ret) {
			hp_err("%s:irq gpio request failed, ret = %d\n", __func__, ret);
			return ret;
		}
		sih_haptic->chip_attr.irq_gpio_flag = true;
	}
	...
	if (gpio_is_valid(sih_haptic->chip_attr.reset_gpio)) {
		...
		if (ret) {
			hp_err("%s:reset gpio request failed, ret = %d\n", __func__, ret);
			return ret;
		}
	}
	...
	const struct i2c_device_id *id)
{
	...
	int ret = -1;
	int cnt = ACQUIRE_GPIO_MAX_RETRIES;
